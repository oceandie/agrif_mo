...
def mpl_sec_get_section(lon3, lat3, rbat2, zenv2, sec_i, sec_j, coord_type, \
    ...
    if (isinstance(rbat2, np.ndarray) and rbat2.size):
    ...
    if (isinstance(var3, np.ndarray) and var3.size): 
       var2 = np.zeros(shape=(nk,sectLen))
    else:
       var2 = None

    if (isinstance(tmsk3, np.ndarray) and tmsk3.size): 
       tmsk2 = np.zeros(shape=(nk,sectLen))
    else:
       tmsk2 = None

    if (isinstance(tdep3, np.ndarray) and tdep3.size): 
       tdep2 = np.zeros(shape=(nk,sectLen))
    else:
       tdep2 = None

    if (isinstance(wdep3, np.ndarray) and wdep3.size):
       wdep2 = np.zeros(shape=(nk,sectLen))
    else:
       wdep2 = None

    if (isinstance(tpnt3, np.ndarray) and tpnt3.size):
       tpnt2 = np.zeros(shape=(nk,sectLen))
    else:
       tpnt2 = None

    if (isinstance(wlev3, np.ndarray) and wlev3.size):
       wlev2 = np.zeros(shape=(nk,sectLen))
    else:
       wlev2 = None

    if (isinstance(var3_aux, np.ndarray) and var3_aux.size):
    ...
    for s in np.arange(sectLen):
        ...
        if var2  is not None: var2[:,s]     = var3[:,j,i]
        if tmsk2 is not None: tmsk2[:,s]    = tmsk3[:,j,i]
        if tdep2 is not None: tdep2[:,s]    = tdep3[:,j,i]
        if wdep2 is not None: wdep2[:,s]    = wdep3[:,j,i]
        if tpnt2 is not None: tpnt2[:,s]    = tpnt3[:,j,i]
        if wlev2 is not None: wlev2[:,s]    = wlev3[:,j,i]
        if var2_aux is not None: var2_aux[:,s] = var3_aux[:,j,i]
        if msk_mes2 is not None: msk_mes2[:,s] = msk_mes[j,i]
        ...
        if rbat2 is not None: ycoordB[s] = rbat2[j,i]
        if zenv2 is not None and zenv2 != []: 
...
def mpl_sec_bathy(rbat, rbat_fill, zenv, x2, tdep2, wdep2, tmsk2, mbat_fill, \
    ...
    if rbat is not None:
       ...
       if tdep2 is not None or wdep2 is not None or tmsk2 is not None:
          rbat_z.append(rbat_max)
          rbat_z.append(rbat_max) 
       if tdep2 is not None and wdep2 is not None and tmsk2 is not None:
    ...
    if tdep2 is not None and wdep2 is not None and tmsk2 is not None and vlevel != "no":

       msk_oce = np.zeros(shape=tmsk2.shape)
       msk_oce[tmsk2 == 0] = 1
       msk_oce[tmsk2 == 1] = np.nan
       if msk_loc is not None:
...
def mpl_lev(exp_name, fig_type, sec_i, sec_j, tlon3, tlat3, tdep3, wdep3, tmsk3, m, \
            ...
            if (isinstance(tdep3, (list, tuple)) and len(tdep3) == 0) or \
               (isinstance(wdep3, (list, tuple)) and len(wdep3) == 0) or \
               (isinstance(tmsk3, (list, tuple)) and len(tmsk3) == 0):
               i3D_flag = False
