...
def generate_loc_area_dept(da_bathy, max_dep, polygon):

    msk_val = -99

    if da_bathy.shape == (139,273):
       # orca025
       i_start = 45
       j_start = 80
       ...
       i_start = 2500
       j_start = 2465
    elif polygon == 3 and da_bathy.shape == (208,248):
       j_start = 100
       i_start = 150
    elif polygon == 3 and da_bathy.shape == (128,152):
       j_start = 60
       i_start = 80
    elif polygon == 3 and da_bathy.shape == (283,328):
       j_start = 150 
       i_start = 190
    else:
    ...
    if polygon == 1: # Gulf Stream
       ...
       LAT = [ 26.,  35. ,  45.5,   45.5,  48.,  47.5,  47., 47.0, 47.0, 26.0, 26.0]
    elif polygon == 3: # MOVF
       #LON = [-10. , -4.7, -4.7,  -10]
       LON = [-10. , -4.22, -4.22,  -10]
       LAT = [ 37.5,  37.4,  34.5, 34.6]
...
def generate_loc_area_grad(da_bat, ds_cor, lim_slp, area):
    ...
    # POLYGON
    if area == 4:
       LON = [-70., -81., -81.5, -71.5, -20.0, -70] 
       LAT = [ 26.,  26.,  35.,  46.0,  46.5,  26]
       #LON = [-70., -81., -83.5, -71.5, -71.0, -46.0, -46.0, -20.0, -70] 
       #LAT = [ 26.,  26.,  35. ,  45.5,  67.0,  66.0,  57.0,  30.0,  26]    
       j_start = 105
       i_start = 55
    elif area == 5:
       LON = [-10. , -4.7, -4.7, -10]
    ...
    # Smoothing for cleaner gradient isolines
    H_T = H_T.where(H_T>1000.,0.)
    #H_T = H_T.where(H_T>500.,0.)
    H_T = gaussian_filter(H_T, sigma=2.)
   
    # Computing gradient components
    dHdx = np.diff(H_T, axis=1, append=np.nan) / ds_cor.e1u # gradient is now on U points
    dHdx = dHdx.rolling({'x':2}).mean().fillna(0.) # back to T points

    dHdy = np.diff(H_T, axis=0, append=np.nan) / ds_cor.e2v # gradient is now on V points
    dHdy = dHdy.rolling({'y':2}).mean().fillna(0.) # back to T points

    grad_H = np.sqrt(dHdx**2 + dHdy**2)
    grad_H = grad_H.where(da_bat>900.).squeeze()
    #grad_H = grad_H.where(da_bat>400.).squeeze()

    #grad_H.plot.contourf(levels=[0., 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.])
    #plt.show()
    ...
    for n in range(len(LON)):
        j, i = get_ij_from_lon_lat(LON[n], LAT[n],
               da_bat.nav_lon, da_bat.nav_lat)
        Js.append(j)
        Is.append(i)

    msk_poly = np.copy(da_bat)*0.
    area_j, area_i = get_poly_area_ij(Is, Js, msk_poly.shape)
    msk_poly[area_j, area_i] = 1

    # FINDING THE PART WE ARE INTERESTED IN
    grad_poly = xr.where(msk_poly == 1, grad_H, msk_val)
    s_msk1 = xr.where(np.logical_and(grad_poly > lim_slp, grad_poly > 0.), 1., 0.)
    s_msk2 = xr.where(msk_poly == 1,s_msk1*-2.,-2.)
    ...
    if area == 4:
       ...
       msk_fill[msk_fill != -1] = 0
       msk_fill *= -1 
       s_msk1.data = msk_fill

    msk = s_msk2.data
    msk_fill = floodfill(msk, j_start, i_start, -2., -1.)
    msk_fill[msk_fill == -2] = 0
    msk_fill *= -1
    s_msk2.data = msk_fill
   
    if area == 4:
       s_msk = xr.where(np.logical_or(s_msk1==1,s_msk2==1), 1., 0.)   
    else:
       s_msk = xr.where(s_msk2==1, 1., 0.) 

    return s_msk

def generate_loc_area_poly(da_bat):

    msk_val = -99

    # POLYGON
    LON = [-81., -81.5, -71.5, -25.0, -25.0, -81.]
    LAT = [ 26.,  35.0,  46.0,  46.5,  26.0,  26.]
    ...
    s_msk = da_bat*0.
    area_j, area_i = get_poly_area_ij(Is, Js, s_msk.shape)
    s_msk[area_j, area_i] = 1
